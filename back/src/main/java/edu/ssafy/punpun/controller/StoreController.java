...
public class StoreController {
    ...
    @GetMapping("/distTestTime/{lon}/{lat}")
    @ResponseStatus(code = HttpStatus.OK)
    public List<StoreInfoResponseDTO> getStoreDistanceTest1(@PathVariable(name = "lon") float lon,
                                                           @PathVariable(name = "lat") float lat,
                                                           @RequestParam(name = "mode", required = false) String mode) {
        List<StoreInfoResponseDTO> storeInfoResponseDTOList = null;
        if (mode.equals("java")) {
            storeInfoResponseDTOList = storeService.getStoreDistanceTestJava(lon, lat).stream()
                    .map(store -> new StoreInfoResponseDTO(store))
                    .collect(Collectors.toList());
        }
        if (mode.equals("postgres")) {
            storeInfoResponseDTOList = storeService.getStoreDistanceTestPostgres(lon, lat).stream()
                    .map(store -> new StoreInfoResponseDTO(store))
                    .collect(Collectors.toList());
        }
    ...
    @ResponseStatus(code = HttpStatus.OK)
    public List<StoreDistResponseDTO> getStoreDistanceTest2(@PathVariable(name = "lon") float lon,
                                                           @PathVariable(name = "lat") float lat,
                                                           @RequestParam(name = "mode", required = false) String mode) {
